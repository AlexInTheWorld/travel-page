...
function showPostCommentForm(button) {
    if (document.querySelector(`.comment#${sessionStorage.getItem("user")}`)) {
        alert("You've already posted a comment for the current destination");
...
function displayComments(json_str) {
    ...
    if (data_len === 0) {
        ...
        for (let i = 0; i < data_len; i++) {            
            ...
            comment_el.setAttribute("id", data[i].user);
            var header_el = document.createElement("P");
            var header_txt = document.createTextNode(data[i].user + " on " + new Date(data[i].date + " UTC").toLocaleString() + " wrote:");
    ...
    // If user has logged in, show button for comment posting
...
city_input.addEventListener("input", async function() {
    ...
    var last_call = await decideResponse(); // Process ajax response when input is empty or remains unchanged for the time defined by extra_interval
    if (last_call) {
...
function disableInputControls() {
    city_input.setAttributeNode(document.createAttribute("disabled")); document.querySelector("button").setAttributeNode(document.createAttribute("disabled"));
}
